...
1. **Objective labels tacked onto sentences** (avoid overuse; prefer integrated flow):
   ...
   - Note: APA (Sec 6.23, 6.49-6.51) does not prohibit parenthetical labels, but weaving the objective's purpose into the sentence demonstrates deeper understanding and reads as stronger academic prose. Use parenthetical labels sparingly, not as a default.
...
1. **One idea per sentence** (APA Sec 4.20-4.23). If a sentence needs parentheses to add a second thought, split it into two sentences.

2. **Vary sentence length** (APA Sec 4.20-4.23). Research writing should not be uniformly long. Alternate between shorter, declarative sentences and longer, elaborative ones. A paragraph of only 30+ word sentences is exhausting to read; a paragraph of only 8-word sentences reads as choppy.

3. **Avoid noun clusters** (APA Sec 4.22). Strings of nouns acting as adjectives ("role-based access control implementation framework architecture") are hard to parse. Break them up with prepositions: "the architecture for implementing role-based access control."

4. **Active voice is preferred where clarity improves** (APA Sec 4.13). APA 7 allows and encourages active voice. "The system generates audit logs" is clearer than "Audit logs are generated by the system." However, passive voice is acceptable when the actor is irrelevant or when the object is more important than the subject.

5. **Use em dashes (—) sparingly and purposefully.** APA 7th edition (Sec 6.83) explicitly permits em dashes. They are not banned but should be used only when commas and parentheses would create confusion. Prefer commas, periods, or sentence restructuring in most cases. If you use an em dash, ask: "Is this the clearest way to present this idea, or am I just avoiding restructuring the sentence?"

6. **Use "respectively" judiciously.** APA does not ban this word (it appears throughout the APA Manual itself), but it often signals a sentence that could be clearer if restructured. Before using "respectively," check: can you simply state each item's relationship directly? "The Guidance Office handles testing, and the Registrar Office manages applications" is clearer than "The Guidance Office and Registrar Office handle testing and applications, respectively." Use "respectively" only when the paired structure is genuinely the most readable option.
...
8. **Do not use "etc." in academic writing** (APA Sec 4.20-4.23). Either list the complete set or use phrasing like "including," "such as," or "among others" followed by representative examples.

### Abbreviation Rules

1. Spell out on first mention, followed by the abbreviation in parentheses (APA Sec 6.24-6.26) — **only if the abbreviated form appears at least 3 more times** in the manuscript.
