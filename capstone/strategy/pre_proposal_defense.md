...
| **"Guidance & Registrar"** | Standard tables, search bars, and printable PDFs. | Enhanced AI Companion with RAG (Mixedbread embeddings) for applicant guidance and course recommendations; AI-Assisted Scheduling (human-in-the-loop). | *"To support applicants navigating the admission process, we enhanced the AI Companion with retrieval-augmented generation, allowing examinees to ask about course recommendations, exam requirements, and institutional data using natural language. For scheduling, an AI assistant generates optimized session proposals based on room capacity and proctor availability, but all actions require explicit registrar approval — keeping the human in the loop."* |
