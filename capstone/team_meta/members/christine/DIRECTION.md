...
2. **Write One Paragraph Per Beneficiary Group** (90-120 min):
   ...
   - Content guidance per group:
     ...
     **Guidance Office counselors.** The system streamlines test administration through session roster management, proctor assignment, and digital attendance tracking. Counselors benefit from automated scoring via OMR CSV import (and planned computer vision ingestion), consultation summary documentation, and aptitude area management. The AI-powered RAG copilot (planned) enables natural-language querying of applicant data, reducing the time spent manually compiling statistics for counseling recommendations. These features allow counselors to focus on student guidance rather than administrative overhead.
