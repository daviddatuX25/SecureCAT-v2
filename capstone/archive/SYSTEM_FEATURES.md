...
*   **ML-Assisted Course Triage and Recommender Module:**
    ...
    *   *Triage Engine* (new logic): Live K-Means clustering engine that executes at classification time, adopting and adapting the validated K=4 configurations from Yukee et al. (2025) and Ballesteros et al. (2025). The system integrates school-owned algorithm source code (ISPSC-produced software from prior capstone teams) tailored for live operation within the SecureCAT Laravel backend. Applies a dual-classification surface — aptitude-based profiles (Language/Math/Spatial/Mixed dominant) from Study 1 and socio-academic profiles (Income × GWA × Area Type clusters) from Study 2. K-Means runs in milliseconds on small datasets (hundreds of students, 5-7 features), making it suitable for live admission workflows. The contribution is the operational integration, not the algorithm itself — the "adopt & tweak" approach is defensible because SecureCAT bridges the gap from research output to live institutional workflow.
