<script>
  ...
  const form = useForm({
    ...
    _method: 'PUT', // For multipart/form-data upload via POST
  });
  ...
  $form.onFinish = () => {
    if (!$form.errors || Object.keys($form.errors).length === 0) {
      success('Admission slip template updated');
    }
  };
  ...
  function submitForm(e) {
    ...
    // We must use POST with _method=PUT to support file uploads in Laravel correctly
    $form.post(`/admin/admission-slip-templates/${template.id}`, { forceFormData: true });
  }
  ...
  $effect(() => {
    ...
    $form.orientation;
    if ($form.mode === 'html' && $form.content) fetchPreview();
    else if ($form.mode === 'docx' && (documentFile || template?.docx_path)) fetchPreview();
  });
</script>
