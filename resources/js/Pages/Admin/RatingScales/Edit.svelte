<script>
  ...
  let { rating_scale } = $props();

  const form = useForm({
    name: rating_scale.name,
    ranges: rating_scale.ranges ?? [{ min: 0, max: 100, label: '' }],
    is_default: rating_scale.is_default,
  });

  $form.onFinish = () => {
    if (!$form.errors || Object.keys($form.errors).length === 0) {
      success('Rating scale updated');
    }
  };
  ...
  function submitForm(e) {
    e.preventDefault();
    $form.put(`/admin/setup/rating-scales/${rating_scale.id}`);
  }
