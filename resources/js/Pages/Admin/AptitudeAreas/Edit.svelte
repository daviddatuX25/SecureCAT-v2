<script>
  ...
  let { aptitude_area, enable_normalized_scores = false } = $props();
  const page = usePage();
  ...
  const form = useForm({
    ...
    scoring_method: aptitude_area.scoring_method ?? 'formula',
    conversion_table: aptitude_area.percentile_conversions ?? [],
