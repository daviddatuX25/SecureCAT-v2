...
## Inertia v2.0 Breaking Changes

- **`remember` → `useRemember`**: Helper renamed for consistency with other helpers
- **Partial reloads**: Now asynchronous (was synchronous in v1)
- **`setup` callback**: Must pass `props` when initializing App component
- **Dropped Svelte 3 support**: Requires Svelte 4+ (Svelte 5 recommended)

## Svelte 5 + Inertia v2 Compatibility

- **Components use Svelte 5 runes**: `$props()`, `$state()`, `$derived()`, `$effect()` work perfectly
- **Inertia helpers return stores**: `useForm()` and `usePage()` return stores requiring `$` prefix (`$form`, `$page`)
- **Library uses Svelte 4 syntax**: @inertiajs/svelte library components use Svelte 4 syntax internally - this is expected and fine
- **Mixed mode supported**: Svelte 5 allows mixing Svelte 4 and 5 syntax, so don't force runes mode globally
...
9. **Empty useForm() call** — `useForm()` requires form field object: `useForm({ email: '', password: '' })`
