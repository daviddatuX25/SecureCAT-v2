...
**Important**: `useForm()` requires an object with form field definitions. It cannot be called without arguments or with an empty object.
...
<script lang="ts">
  ...
  // Always provide form fields object
  const form = useForm({
    email: '',
    password: '',
    remember: false,
...
**Note**: In Inertia v2.0, partial reloads are now **asynchronous** (were synchronous in v1). Don't rely on synchronous behavior; use callbacks if needed.
...
## useRemember (State Persistence)

Save local component state to browser history so it's restored when navigating back/forward.
...
<input bind:value={form.first_name} />
<input bind:value={form.last_name} />
```

### With useForm

If using `useForm()`, pass a unique key as the first argument to automatically remember form data and errors:

```svelte
<script lang="ts">
  import { useForm } from '@inertiajs/svelte';
  
  // Form data and errors automatically remembered
  const form = useForm('CreateUser', {
    name: '',
    email: '',
  });
  
  // For edit forms, include model ID in key
  const editForm = useForm(`EditUser:${user.id}`, {
    name: user.name,
    email: user.email,
  });
</script>
...
Use `dontRemember()` to exclude sensitive fields (like passwords) from history state:
...
<script lang="ts">
  ...
  // Password won't be saved to history
...
If multiple components use `useRemember`, provide unique keys:

```svelte
<script lang="ts">
  import { useRemember } from '@inertiajs/svelte';
  
  const form1 = useRemember({ name: '' }, 'Users/Create');
  const form2 = useRemember({ title: '' }, 'Posts/Create');
</script>
```
...
Load non-essential data after initial page load:
...
public function index()
{
    return Inertia::render('Station/Index', [
        ...
        // Only loaded when router.reload({ only: ['sessions'] }) is called
        'sessions' => Inertia::lazy(fn () => 
            Session::where('station_id', $station->id)->get()
        ),
    ]);
...
Automatically prefetch data when links become visible:
...
<!-- Prefetches /dashboard when link becomes visible -->
